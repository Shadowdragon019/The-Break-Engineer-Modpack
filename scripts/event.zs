		...
		if (block == <block:kubejs:active_zapper>) {
			...
			if (stack.damage >= stack.maxDamage - 1) {
				...
				if (level is ServerLevel) {
					...
					(level as ServerLevel).server.executeCommand("function the_breaking_engineer:events/new_dialogue_message", true);
					(level as ServerLevel).server.executeCommand("schedule function the_breaking_engineer:delayed_dialogue/energy_control_room/defeat_fairy_1 1s", true);
					(level as ServerLevel).server.executeCommand("schedule function the_breaking_engineer:delayed_dialogue/energy_control_room/defeat_fairy_2 2s", true);
